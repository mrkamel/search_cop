...
class VisitorTest < SearchCop::TestCase
  ...
  def test_matches
    ...
    assert_equal("COALESCE(#{quote_table_name "products"}.#{quote_column_name "notice"} LIKE #{quote "%Notice%"}, 0)", SearchCop::Visitors::Visitor.new(ActiveRecord::Base.connection).visit(node)) if ENV["DATABASE"] != "postgres"
    assert_equal("COALESCE(#{quote_table_name "products"}.#{quote_column_name "notice"} ILIKE #{quote "%Notice%"}, 0)", SearchCop::Visitors::Visitor.new(ActiveRecord::Base.connection).visit(node)) if ENV["DATABASE"] == "postgres"
  end
